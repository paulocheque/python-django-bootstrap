...
task :bootstrap => [] do
  sh "git remote add origin git@bitbucket.org:paulocheque/{{ project_name }}.git"
  sh "git remote add bootstrap git@github.com:paulocheque/python-django-bootstrap.git"
  sh "virtualenv env -p pypy"
  virtual_env("pip install -r requirements.txt", "env")
  virtual_env("pip install -r requirements-dev.txt", "env")
  virtual_env("python manage.py migrate", "env")
  virtual_env("python manage.py createsuperuser", "env")
  virtual_env("python manage.py runserver", "env")
...
task :server => [] do
  virtual_env("PORT=8000 foreman start")
end
...
namespace :heroku do
  ...
  task :create => [] do
    ...
    sh "heroku stack:set cedar-14 --app #{SERVER}" if SERVER
    sh "heroku stack:set cedar-14 --app #{WORKER}" if WORKER
  ...
  task :config => [] do
    sh "heroku stack --app #{SERVER}" if SERVER
    sh "heroku stack --app #{WORKER}" if WORKER
